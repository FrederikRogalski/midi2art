...
private:
    ...
    int currentProtocol = 1;       // 0 = Art-Net, 1 = E1.31 (default)
    ...
    // Update rate for Art-Net when notes are active (send at ~40Hz)
    int updateCounter = 0;
    static constexpr int UPDATE_INTERVAL = 1103; // 44100 / 40 (samples between updates)
    
    // Previous LED count for visual feedback
    int previousLEDCount = 0;
    
    // Maximum LED count ever set (to ensure we always send packets covering the full range)
    int maxLEDCountEver = 0;
